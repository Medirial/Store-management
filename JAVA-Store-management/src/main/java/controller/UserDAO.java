...
public class UserDAO {
    ...
    public String[] readAllAttendants() throws SQLException{
        int rows = 0;
        resultCountRows = connection.prepareStatement("SELECT COUNT(*) FROM User WHERE access_level = 'Seller'").executeQuery();
        if(resultCountRows.next()) rows = resultCountRows.getInt(1);

        query = "SELECT full_name FROM User WHERE access_level = 'Seller'";
        statement = connection.prepareStatement(query);
        result = statement.executeQuery();
